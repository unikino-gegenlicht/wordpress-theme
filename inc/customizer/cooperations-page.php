...
class CooperationPage extends GGLCustomizerBase {
	const SECTION = 'page_cooperation';
	...
	private function add_controls(): void {
		$this->manager->add_control( "cooperation_page_title[de]", array(
			"section" => self::SECTION,
			"label"   => __( 'Heading (German)', 'gegenlicht' ),
		) );

		$this->manager->add_control( "cooperation_page_title[en]", array(
			"section" => self::SECTION,
			"label"   => __( 'Heading (English)', 'gegenlicht' ),
		) );

		$this->manager->add_control( "cooperation_page_text[de]", array(
			"section" => self::SECTION,
			"label"   => __( 'Introduction (German)', 'gegenlicht' ),
			"type"    => "textarea"
		) );

		$this->manager->add_control( "cooperation_page_text[en]", array(
			"section" => self::SECTION,
			"label"   => __( 'Introduction (English)', 'gegenlicht' ),
			"type"    => "textarea"
		) );

		$this->manager->add_control( new GGL\customizer\MultiInputControl( $this->manager, "cooperation_rules[de]", array(
			"section" => self::SECTION,
		...
		$this->manager->add_control( new GGL\customizer\MultiInputControl( $this->manager, "cooperation_rules[en]", array(
			"section" => self::SECTION,
		...
		$this->manager->add_control( "cooperation_partner_text[de]", array(
			"section" => self::SECTION,
			"label"   => __( 'Partner List Text (English)', 'gegenlicht' ),
			"type"    => "textarea"
		) );

		$this->manager->add_control( "cooperation_partner_text[en]", array(
			"section" => self::SECTION,
			"label"   => __( 'Partner List Text (English)', 'gegenlicht' ),
			"type"    => "textarea"
		) );
