...
class CooperationAndSupportersBlockCustomizer extends GGLCustomizerBase {
	...
	function __construct( WP_Customize_Manager $manager, int $priority = 20, array $additionalArgs = [] ) {
		parent::__construct( $manager, __( 'Cooperation Partners and Supporters', 'gegenlicht' ), __( 'Configure the content of the block showcasing our supporters and cooperation partners', 'gegenlicht' ), priority: $priority, section: self::SECTION, additionalArgs: array_merge( $additionalArgs, [ "active_callback" => function() {return $this->is_block_enabled();} ] ) );
	...
	private function register_controls(): void {
		...
		$this->manager->add_control(new WP_Customize_Color_Control($this->manager, "coop_block_text_color[light]", array(
			"section"     => self::SECTION,
		...
		$this->manager->add_control(new WP_Customize_Color_Control($this->manager, "coop_block_bg_color[light]", array(
			"section"     => self::SECTION,
		...
		$this->manager->add_control(new WP_Customize_Color_Control($this->manager, "coop_block_text_color[dark]", array(
			"section"     => self::SECTION,
		...
		$this->manager->add_control(new WP_Customize_Color_Control($this->manager, "coop_block_bg_color[dark]", array(
			"section"     => self::SECTION,
