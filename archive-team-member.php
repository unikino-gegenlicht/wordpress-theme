                ...
                foreach ( $members['active'] as $member ) : $imageUrl = get_the_post_thumbnail_url( post: $member, size: 'member-crop' );
                    ...
                    <div class="cell">
                        <figure class="image is-3by4 member-picture">
                            <img alt=""
                                 src="<?= $imageUrl ?: wp_get_attachment_image_url( get_theme_mod( 'anonymous_team_image' ), size: 'member-crop' ) ?>"/>
                        </figure>
                        <hr class="separator mb-1"/>
                        <a href="<?= get_permalink( $member ) ?>"><h5 class="is-size-5"><?= $member->post_title ?></h5>
                        </a>
                    ...
                    foreach ( $members['former'] as $member ) : $imageUrl = get_the_post_thumbnail_url( post: $member, size: 'member-crop' );
                        ...
                        <div class="cell">
                            ...
                            <a href="<?= get_permalink( $member ) ?>"><h5
                                        class="is-size-5"><?= $member->post_title ?></h5></a>
                            <p class="is-italic"> <?= rwmb_meta( 'joined_in', post_id: $member->ID ) ?>
                                –<?= rwmb_meta( 'left_in', post_id: $member->ID ) ?> </p>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
